...
- bottom_line = **2-3 משפטים תמציתיים** בעברית (לא משפט יחיד), **בטון עיתונאי ממוקד**. כתוב כמו כתב כלכלי ב-TheMarker:
  • משפט 1 — העובדה המרכזית: פתח בפועל חזק ("החברה זכתה ב...", "הדירקטוריון אישר...", "מאזן הרבעון מצביע על...") וכלול את הנתון המרכזי (סכום, אחוז, שם חברה שנרכשה)
  • משפט 2 — הקשר או רקע: על איזו תקופה/פעילות מדובר, מה קדם, או נתון משלים מהכותרת
  • משפט 3 (אופציונלי) — המשמעות למשקיע: מה זה אומר על המניה/הענף, בלי לנחש מספרים
  • אסור: גוף ראשון, "אני", שיפוט חשיבות ("שגרתי", "משמעותי"), שלילה ("אינה מכילה", "ללא השפעה")
  • אל תמציא עובדות שאינן בכותרת — אם המידע דל, הסתפק ב-2 משפטים
...
דוגמאות bottom_line איכותיות (2-3 משפטים):
- "ברן זינקה 12% לאחר שחתמה על מזכר הבנות לפרויקט התפלת מים בהיקף 150 מיליון דולר באפריקה. ההסכם, מהגדולים שחתמה החברה בתחום, צפוי להיפרס על פני מספר שנים. זכייה בפרויקט מסדר גודל כזה מחזקת את צבר ההזמנות שלה לטווח הבינוני."
- "הדירקטוריון אישר חלוקת דיבידנד של 0.45 שקל למניה בגין הרבעון הראשון. זוהי חלוקה שנייה ברציפות לאחר שנת שיא. המהלך משקף ביטחון ההנהלה בתזרים המזומנים השוטף."
- "בזק הגישה לנאמן דוחות כספיים שנתיים לסדרות אג\"ח 11-14 לשנת 2025. מדובר בדיווח תקופתי שגרתי במסגרת התחייבויות הגילוי כלפי מחזיקי האג\"ח."
  ...
  try {
    ...
    const bottom_line =
      typeof out.bottom_line === 'string' && out.bottom_line.trim()
        ? out.bottom_line.trim().slice(0, 500)
        : null
