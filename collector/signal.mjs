...
- bottom_line = משפט אחד קצר בעברית, בטון עיתונאי יבש וענייני, בלי גוף ראשון ובלי המילה "אני", שמתאר **בחיוב** מה הדיווח כולל / על מה הוא. תאר רק את מה שהדיווח **כן** מכיל — לעולם אל תכתוב מה הוא חסר, מה הוא איננו, או שאין בו מידע מהותי. אסורים לחלוטין ניסוחים כמו "שגרתי", "טכני", "לא משמעותי", "אינו משפיע", "אינה מהווה שינוי", "אינה מכילה מידע מהותי", "ללא השפעה" — הציון כבר מבטא את המהותיות; ה-bottom_line רק מתאר את התוכן עצמו. דוגמה לדיווח על מצגת: "מצגת תקופתית שבה החברה מציגה למשקיעים את פעילותה ותוצאותיה האחרונות".
- אל תמציא עובדות שאינן בכותרת. אם המידע דל — תאר בקצרה את סוג הדיווח לפי הכותרת (למשל "דוח הצעת מדף להנפקת אגרות חוב"), בלי להעריך את חשיבותו ובלי שלילה.
...
- מצגת המסכמת דוחות כספיים / רבעון / תוצאות / "מצגת לשוק ההון" = סיכום ביצועים מהותי, ציון 7-8 (לא נמוך). רק מצגת שיווקית גנרית בלי תוצאות ("מצגת משקיעים", "מצגת פעילות") = 4-5.
